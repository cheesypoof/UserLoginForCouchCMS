...
<cms:else/>
	<cms:form anchor='0' action=k_template_link anchor='0' masterpage='users.php' method='post' mode='create' name='register'>
		...
		<cms:else/>
			...
			<cms:hide>
				<cms:input label='Password' name='password' required='1' type='password' validator='min_len=6 | max_len=64 | matches_field=password_repeat'/>
			</cms:hide>

			<br/>

			<div><label for="password_repeat">Repeat Password</label></div>
			<div><input autocomplete="off" id="password_repeat" maxlength="64" name="password_repeat" pattern=".{6,64}" required="required" title="6 to 64 characters" type="password" value=""/></div>

			<cms:hide>
				<cms:input label='Repeat Password' name='password_repeat' required='1' type='password' validator='min_len=6 | max_len=64 | matches_field=password'/>
			</cms:hide>
